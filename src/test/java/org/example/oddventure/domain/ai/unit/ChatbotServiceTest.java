...
public class ChatbotServiceTest {
    ...
    @Test
    @DisplayName("툴 적용 메서드가 정상 작동한다.")
    void replySchedule_success() {
        // given
        Long userId = 1L;
        String userMessage = "오늘은 어떤 경기가 있나요?";
        when(chatHistoryService.getRecentMessages(userId)).thenReturn(List.of("이전대화1", "이전대화2"));
    ...
    void classifyTools_success() {
        ...
        // when
        String reply = chatbotService.classifyTools(userMessage);

        // then
        assertThat(reply).isNotNull();
        assertThat(reply).contains("schedule");
    }
     ...
     */
    private void mockChatClientContentWithTool(String content) {
        ChatClient.ChatClientRequestSpec requestSpec = mock(ChatClient.ChatClientRequestSpec.class);
        ChatClient.CallResponseSpec call = mock(ChatClient.CallResponseSpec.class);

        when(chatClient.prompt(anyString())).thenReturn(requestSpec);
        when(requestSpec.system(anyString())).thenReturn(requestSpec);
        when(requestSpec.tools(any())).thenReturn(requestSpec); // tool 개수만큼
